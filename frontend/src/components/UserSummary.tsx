...
export const UserSummary: React.FC<UserSummaryProps> = ({ onAddLog }) => {
  ...
  return (
    <div className="glass-panel glass-panel-hover rounded-2xl p-6 shadow-xl flex flex-col justify-between">
      <div>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-5">
          <input
            ...
            disabled={loading}
            className="flex-1 px-4 py-2.5 rounded-xl bg-white border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none text-slate-800 placeholder-slate-400 text-sm transition-all"
          />
          <button
            type="submit"
            disabled={loading}
            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-200 disabled:text-slate-400 text-white text-sm font-semibold rounded-xl transition-all shadow-md active:scale-95 flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed"
          >
            {loading ? (
        ...
        {/* Results Area */}
        {errorText && (
          <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-sm mb-2 flex items-start gap-2.5 animate-fadeIn">
        ...
        {userData ? (
          <div className="space-y-4 animate-fadeIn">
            <div className="flex justify-between items-center bg-emerald-50/20 p-3 rounded-xl border border-emerald-100/50">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-emerald-50/30 p-3 rounded-xl border border-emerald-100/40 flex flex-col">
          ...
          !errorText && (
            <div className="text-slate-400 text-center py-6 text-sm italic border border-dashed border-slate-200 rounded-2xl bg-white/40">
